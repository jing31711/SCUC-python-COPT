...
int main(int argc, char* argv[])
{
  try
  {
    ...
    const int buildLimit = 8;

    const int nCity = 30;
    vector<string> names;
    for (int i = 0; i < nCity; i++)
    {
      names.push_back("city" + to_string(i));
    }

    vector<int> supplies = {19, 13, 20, 18, 23, 25, 22, 28, 18, 16, 17, 14, 22, 13, 18, 20, 15, 23, 25, 19, 18, 19, 22,
      22, 21, 19, 17, 21, 15, 25};

    map<string, int> mapSupply;
    for (int i = 0; i < nCity; i++)
    {
      mapSupply[names[i]] = supplies[i];
    }

    vector<int> demands = {4, 1, 2, 7, 9, 9, 3, 9, 5, 2, 4, 1, 9, 3, 1, 7, 5, 9, 2, 4, 9, 6, 5, 4, 5, 6, 8, 9, 5, 6};

    map<string, int> mapDemand;
    for (int i = 0; i < nCity; i++)
    {
      mapDemand[names[i]] = demands[i];
    }

    vector<vector<int>> shipCosts = {
      {0, 61, 35, 35, 53, 16, 11, 58, 28, 34, 43, 18, 48, 30, 68, 15, 27, 53, 26, 32, 13, 41, 54, 36, 51, 68, 29, 20, 49, 48},
    ...
    map<string, map<string, int>> mapShipCost;
    for (int i = 0; i < nCity; i++)
    {
      for (int j = 0; j < nCity; j++)
      {
        mapShipCost[names[i]][names[j]] = shipCosts[i][j];
      }
    }

    map<string, double> mapMult;
    for (int i = 0; i < nCity; i++)
    {
      mapMult[names[i]] = 0.0;
    }

    map<string, double> mapSlack;
    for (int i = 0; i < nCity; i++)
    {
      mapSlack[names[i]] = 0.0;
    }
    ...
    Model model = env.CreateModel("locationtransport_lr");
    ...
    for (int i = 0; i < nCity; i++)
    {
      string name = "build_" + names[i];
      vbuild.PushBack(model.AddVar(0, 1, 0, COPT_BINARY, name.c_str()));
    }

    VarArray vship[nCity];
    for (int i = 0; i < nCity; i++)
    {
      for (int j = 0; j < nCity; j++)
      {
        string name = "ship_" + names[i] + "_" + names[j];
        vship[i].PushBack(model.AddVar(0, demands[j], 0, COPT_INTEGER, name.c_str()));
      }
    }
    ...
    for (int i = 0; i < nCity; i++)
    {
      ...
      string name = "supply_" + names[i];
      csupply.PushBack(model.AddConstr(expr <= mapSupply[names[i]] * vbuild[i], name.c_str()));
    }
    ...
    for (int j = 0; j < nCity; j++)
    {
      ...
      string name = "relax_" + names[j];
      crelax.PushBack(model.AddConstr(expr >= mapDemand[names[j]], name.c_str()));
    }

    Expr expr;
    for (int i = 0; i < vbuild.Size(); i++)
    {
      expr += vbuild[i];
    }

    Constraint climit = model.AddConstr(expr <= buildLimit, "Limit");
    ...
    for (int i = 0; i < nCity; i++)
    {
      for (int j = 0; j < nCity; j++)
      {
        obj += vship[i][j] * mapShipCost[names[i]][names[j]];
      }
    }
    model.SetObjective(obj, COPT_MINIMIZE);

    // Solve the relaxed UB problemand initialize 'LB' with relaxed objective value
    model.SolveLp();
    double LB = model.GetDblAttr(COPT_DBLATTR_LPOBJVAL);

    // Initialize 'UB'
    double UB = 0.0;
    for (int i = 0; i < nCity; i++)
    {
      UB += max_element(mapShipCost[names[i]].begin(), mapShipCost[names[i]].end(),
    ...
    string tabs = "        ";
    cout << "*********** Lagrangian-Relaxation Loop ***********" << endl;
    cout << "  Iter" + tabs + "LB" + tabs + "    UB" + tabs + "scale" + tabs + "step" << endl;
    for (int itercnt = 0; itercnt < iterLimit; itercnt++)
    {
      ...
      for (int i = 0; i < nCity; i++)
      {
        for (int j = 0; j < nCity; j++)
        {
          lbObj += vship[i][j] * mapShipCost[names[i]][names[j]];
        }
      }
      ...
      for (int j = 0; j < nCity; j++)
      {
        ...
        lbObj += mapMult[names[j]] * (mapDemand[names[j]] - expr);
      }

      model.SetObjective(lbObj, COPT_MINIMIZE);
      ...
      for (int j = 0; j < nCity; j++)
      {
        ...
        for (int i = 0; i < nCity; i++)
        {
          value += vship[i][j].Get(COPT_DBLINFO_VALUE);
        }
        mapSlack[names[j]] = value - mapDemand[names[j]];
      }

      // Improve 'LB'
      if (model.GetDblAttr(COPT_DBLATTR_BESTOBJ) > LB + 1e-6)
      {
        LB = model.GetDblAttr(COPT_DBLATTR_BESTOBJ);
        nSame = 0;
      }
      else
      {
      ...
      for (int i = 0; i < nCity; i++)
      {
        norm += pow(mapSlack[names[i]], 2);
      }

      // Update 'step'
      step = scale * (UB - model.GetDblAttr(COPT_DBLATTR_BESTOBJ)) / norm;

      // Update 'mapMult'
      for (int i = 0; i < nCity; i++)
      {
        double value = mapMult[names[i]] - step * mapSlack[names[i]];
        mapMult[names[i]] = value >= 0.0 ? value : 0.0;
      }
      ...
      for (int i = 0; i < nCity; i++)
      {
        sumSupply += mapSupply[names[i]] * vbuild[i].Get(COPT_DBLINFO_VALUE);
      }

      double sumDemand = 0;
      for (auto it : mapDemand)
      {
        sumDemand += it.second;
      }
      ...
      if (sumSupply - sumDemand >= 1e-6)
      {
        ...
        // Retrieve solution for the LB problemand fix the variables
        vector<double> solBuild(vbuild.Size());

        model.Get(COPT_DBLINFO_VALUE, vbuild, solBuild.data());

        model.Set(COPT_DBLINFO_LB, vbuild, solBuild.data(), solBuild.size());
        model.Set(COPT_DBLINFO_UB, vbuild, solBuild.data(), solBuild.size());
        ...
        for (int i = 0; i < nCity; i++)
        {
          for (int j = 0; j < nCity; j++)
          {
            ubObj += vship[i][j] * mapShipCost[names[i]][names[j]];
          }
        }
        model.SetObjective(ubObj, COPT_MINIMIZE);

        // Solve the UB problem
        model.Solve();

        // Update 'UB'
        UB = min(UB, model.GetDblAttr(COPT_DBLATTR_BESTOBJ));

        // Reset variables to initial bounds
        for (int i = 0; i < vbuild.Size(); i++)
        {
          vbuild[i].Set(COPT_DBLINFO_LB, 0.0);
          vbuild[i].Set(COPT_DBLINFO_UB, 1.0);
        }
      }

      // Print statistics
      cout << "   " << itercnt << tabs << LB << tabs << UB;
      cout << tabs << scale << tabs << step << endl;
    }
