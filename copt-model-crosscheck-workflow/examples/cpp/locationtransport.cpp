...
int main(int argc, char* argv[])
{
  try
  {
    ...
    // Create COPT problem
    Model model = env.CreateModel("locationtransport");
    ...
    for (int i = 0; i < nCity; i++)
    {
      for (int j = 0; j < nCity; j++)
      {
        ...
        vship[i].PushBack(model.AddVar(0, COPT_INFINITY, 0, COPT_INTEGER, name.c_str()));
      }
    }
    ...
    for (int i = 0; i < nCity; i++)
    {
      ...
      model.AddConstr(expr <= mapSupply[names[i]] * vbuild[i], name.c_str());
    }
    ...
    for (int j = 0; j < nCity; j++)
    {
      ...
      string name = "demand_" + names[j];
      model.AddConstr(expr >= mapDemand[names[j]], name.c_str());
    }
    ...
    model.AddConstr(expr <= buildLimit, "Limit");
    ...
    if (model.GetIntAttr(COPT_INTATTR_HASMIPSOL))
    {
      ...
      for (int i = 0; i < vbuild.Size(); i++)
      {
        if (vbuild[i].Get(COPT_DBLINFO_VALUE) > 1e-6)
        {
          cout << "Build warehouse in " << names[i] << endl;
        }
      }
      cout << endl;
      for (int i = 0; i < nCity; i++)
      {
        for (int j = 0; j < nCity; j++)
        {
          if (vship[i][j].Get(COPT_DBLINFO_VALUE) > 1e-6)
          {
            cout << "Ship " << vship[i][j].Get(COPT_DBLINFO_VALUE) << " items from ";
            cout << names[i] << " to " << names[j] << endl;
          }
        }
      }
    }
